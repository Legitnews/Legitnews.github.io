...
function grayscale(ctx){
    ...
    for (var i=(canvas.width * canvas.height * 4) - 4; i>=0; i -= 4){
        gray = (imageData.data[i] * 0.2126 + imageData.data[i+1] * 0.7152 + imageData.data[i+2] * 0.0722);
...
function Bullet(pos, dir, owner, colour, radius, damage){
    ...
    this.vel = this.dir.copy().mul(this.speed);
    ...
    this.update = function(){
        ...
        this.translate(this.vel.copy().mul(deltaTime));
    }
...
function drawLava(colour){
	if (Game.player.collider.pos[1] + canvas.height / 2 - Game.totalDelta[1] >= Game.deathDepth){
		
		ctx.fillStyle = colour;
		
		var start = Game.deathDepth + Game.totalDelta[1] + Game.player.collider.size[1];
...
function Player(pos, size){
    ...
    this.fireSound = document.getElementById("fire"); 
    this.reloadSound = document.getElementById("reload");
    ...
    this.fireSound.addEventListener('ended', function() {
        Game.player.reloading = true;
        Game.player.reloadSound.play();
    }, false);

    this.reloadSound.addEventListener('ended', function() {
        Game.player.reloading = false;
        Game.player.canFire = true;
    }, false);

    this.lose.addEventListener('ended', function() {
        Game.gameOver();
    }, false);
    ...
    this.update = function(){
        ...
        if (!this.canFire && !(this.reloading || this.fireSound.currentTime > this.firingTime)){
            this.fire();
        }
    ...
    this.fire = function(){
        ...
        var b = new Bullet(this.collider.centre, angle, this);

        b.vel.add(this.velocity);
    ...
    this.handleKey = function(){
        ...
        if (Key.isDown(Key.SPACE) || Key.isDown(Key.UP) || Key.isDown(Key.W)){
            this.jump();
        }
    }
    ...
    this.draw = function(){
        ...
        if (this.dead){
            if (this.grayscale !== null){
                ctx.drawImage(this.grayscale, 0, 0);
            }
        ...
        if (mousePos[0] < canvas.width / 2){
			this.sprite.facing = facings.LEFT;
		}
		else if (mousePos[0] > canvas.width / 2){
...
Bat.size = [20, 15];
function Bat(pos){
    Enemy.call(this, pos, Bat.size, "#000000", 25);
...
Moustachiotelli.size = [20, 50];
function Moustachiotelli(pos){
	Runner.call(this, pos);
	
	this.right_images = [
        document.getElementById("Moustachiotelli0"),
        document.getElementById("Moustachiotelli1"),
    ]
    this.left_images = [
        document.getElementById("Moustachiotelli2"),
        document.getElementById("Moustachiotelli3"),
    ]
...
function Monster(pos){
    ...
    this.REKT = document.getElementById("REKT");

    this.REKT.addEventListener('ended', function() {
        this.currentTime = 0;
        Game.music.play();
    }, false);
    
    this.eyePos = new Vector2(320, 200);
    ...
    this.fire = function(){
        ...
        return new Bullet(firePos, angle, this, "#FF6A00", 9);
    }
...
var Game = {
    ...
    enemyTypes : [Runner, Runner, Runner, Runner, Runner.
    ...
    update : function(){
        ...
        if (this.mode === "timed"){
			this.deathDepth -= (this.deathDepth / this.trialTime) * deltaTime;
		}
    ...
    render : function(){
        ...
        if (this.player.dead && this.player.grayscale !== null){
            this.player.draw();
            return;
        }
        ...
        if (this.player.dead && this.player.grayscale === null){
            this.player.grayscale = grayscale(ctx);
...
window.setInterval(function(){ Game.checkParticles(); }, 100)
