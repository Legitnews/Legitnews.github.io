...
function Grid(topleft, size, tileSize, canvas, defaultType, defaultRender, intToTileRender){
	...
	this.defaultType = defaultType ? defaultType : 0;
	this.defaultRender = defaultRender ? defaultRender : 0;

	this.tileTypes = [];
	this.tileRenderTypes = [];

	this.animationImported = (typeof AnimationThread !== undefined);

	for (var i=0; i < this.size[0]; i++){
		this.tileTypes.push([]);
		this.tileRenderTypes.push([]);

		for(var j=0; j < this.size[1]; j++){
			this.tileTypes[i][j] = this.defaultType;
			this.tileRenderTypes[i][j] = this.defaultRender;
		}
	}

	this.fillDefault = function(type){
		type = type === undefined ? this.defaultType : type;

		for (var i=0; i < this.size[0]; i++){
			for(var j=0; j < this.size[1]; j++){
				this.tileTypes[i][j] = type;
			}
		}
	};

	this.fillRender= function(type){
		type = type === undefined ? this.defaultRender : type;

		for (var i=0; i < this.size[0]; i++){
			for(var j=0; j < this.size[1]; j++){
				this.tileRenderTypes[i][j] = type;
			}
		}
	};

	this.tileContext = function(type, type2, surroundingToNewRender){
		/*
		 * up : 1000
		 * right : 0100
		 * left : 0010
		...
		for (var i=0; i < this.size[0]; i++){
			for(var j=0; j < this.size[1]; j++){
				...
				if (tile[1] > 0){
					sTileOfType += this.tileTypes[tile[0]][tile[1] - 1] === type2 ? "1" : "0";
				}
				else{
					sTileOfType += "0";
				}
				if (tile[0] > 0){
					sTileOfType += this.tileTypes[tile[0] - 1][tile[1]] === type2 ? "1" : "0";
				}
				else{
					sTileOfType += "0";
				}
				...
				if (sTileOfType in surroundingToNewRender){
					this.tileRenderTypes[i][j] = surroundingToNewRender[sTileOfType];
				}
			}
		}
	}
	...
	this.getTileType = function(tile){
		if (tile[0] < 0 || tile[1] < 0 || tile[0] >= this.size[0] || tile[1] >= this.size[1]){
			return -1
		}
	...
	this.getRenderType = function(tile){
		if (tile[0] < 0 || tile[1] < 0 || tile[0] >= this.size[0] || tile[1] >= this.size[1]){
			return -1
		}

		return this.tileRenderTypes[tile[0]][tile[1]];
	}
	...
	this.setTileRenderType = function(tile, type){
		this.tileRenderTypes[tile[0]][tile[1]] = type;
	}
	...
	this.fillTiles = function(ctx, scale){
		...
		for (var i=start_i; i < end_i; i++){
			...
			for(var j=start_j; j < end_j; j++){
				...
				var type = this.tileRenderTypes[i][j];
				...
				if (typeof render === "string"){
					...
					if (render instanceof Animator || render instanceof Animation){
						...
						if (scale){
							ctx.drawImage(render, x, y, this.tileSize[0], this.tileSize[1]);
						}
						else{
